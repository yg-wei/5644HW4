...
def plot_data(model, data, outfile=None):
    if outfile is None:
    ...
    #using matplotlib
    counterplot = plt.contour(
        X, Y, Z, norm=LogNorm(vmin=1.0, vmax=1000.0), levels=np.logspace(0, 3, 10)
    )
    CB = plt.colorbar(counterplot, shrink=0.8, extend="both")
...
def kfold_exp(k_compoonents: int, data: np.ndarray, nfolds=10) -> float:
    data = data.T  # oop
...
def component_selection(components_to_check: List[int], data: np.ndarray, test_data: np.ndarray, plot_selections=False) \
    ...
    for component in components_to_check:
        ...
        comp_to_testLLH[component] = eval_gm_loglikeli(component_model,
                                                       test_data)
        if plot_selections:
            plot_data(component_model, data,
